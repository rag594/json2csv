    ...
    for j in range(len(hid_dir)):
        ...
        if (hid_dir[j] == '.DS_Store') or (hid_dir[j] == '._.DS_Store') or hid_dir[j] == '.DS_Store':
            ...
            for k in range(len(data)):
                ...
                name = (data[k]['name']).encode('ascii', 'ignore').decode('ascii')
                writer.writerow([hid,typ,name,data[k]['geometry']['location']['lat'],data[k]['geometry']['location']['lng'],data[0]['place_id']])
